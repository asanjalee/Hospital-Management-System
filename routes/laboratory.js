...
function generateLabRequestNumber() {
    ...
    const countResult = queryOne('SELECT COUNT(*) as total FROM lab_requests');
...
router.get('/', (req, res) => {
    ...
    try {
        const labRequests = queryAll(sql, params) || [];

        const stats = {
            total: queryOne('SELECT COUNT(*) as cnt FROM lab_requests')?.cnt || 0,
            pending: queryOne('SELECT COUNT(*) as cnt FROM lab_requests WHERE status = "Pending"')?.cnt || 0,
            inProgress: queryOne('SELECT COUNT(*) as cnt FROM lab_requests WHERE status = "In Progress"')?.cnt || 0,
            completed: queryOne('SELECT COUNT(*) as cnt FROM lab_requests WHERE status = "Completed"')?.cnt || 0
        };
...
router.get('/request', (req, res) => {
    ...
    try {
        const autoNumber = generateLabRequestNumber();
...
router.post('/request', [
    ...
    if (!errors.isEmpty()) {
        ...
        return res.render('laboratory/request', {
            title: 'Request Laboratory Test',
            activeMenu: 'laboratory',
            autoNumber: req.body.request_number || generateLabRequestNumber(),
            patients,
            doctors,
    ...
    try {
        const reqNum = request_number && request_number.trim() ? request_number.trim() : generateLabRequestNumber();
        ...
        auditLog(req.session.user.id, 'LAB_REQUEST_CREATED', 'lab_requests', result.lastInsertRowid, `Ordered lab test ${reqNum} (${test_name}) for patient ID #${patient_id}`, req.ip);
...
router.post('/status/:id', (req, res) => {
    ...
    try {
        const lab = queryOne('SELECT request_number FROM lab_requests WHERE id = ?', [labId]);
        if (!lab) {
        ...
        auditLog(req.session.user.id, `LAB_REQUEST_${status.toUpperCase()}`, 'lab_requests', labId, `Lab request ${lab.request_number} status set to ${status}`, req.ip);
